...
function truncate(a,b){a=a.substr(0,b-1);return a.substr(0,a.lastIndexOf(" ")).replace(/(^[,\s]+)|([,\s]+$)/g,"")+"..."}function toTitleCase(a){return a.replace(/\b\w+/g,function(a){return a.charAt(0).toUpperCase()+a.substr(1).toLowerCase()})}function removeFromArray(a,b){-1!==a.indexOf(b)&&a.splice(a.indexOf(b),1)}var albumID=window.location.pathname.replace("/album/",""),albumResult,selectedTags=[],userLists=[];
function getAlbumDetails(a){$.ajax("/api/v1/album/"+albumID,{method:"GET",success:function(b){b.message&&"No album found"===b.message.slice(0,14)?$.getJSON("/api/v1/apple/details/"+albumID,function(b){b.message?alert(b.message):(albumResult=b,populateAlbumPage(a))}):(albumResult=b,populateAlbumPage(a))}})}
...
a+"</li>")});$("#apple-album-id span").text(albumResult.appleAlbumID);$("#more-by-this-artist").click(function(a){a.preventDefault();$("#searchModal").modal("show");$("#search-modal-input").val(albumResult.artist);executeSearch(albumResult.artist)});$("#apple-music-link").click(function(a){a.preventDefault();return window.open(albumResult.appleURL,"_blank").location});populateTags();populateConnections();getUserLists();a?(updateTagDisplay(a),updateConnectionDisplay(a),populateListsWithAlbum(a)):(0===
$(".album-tag").length&&$("#current-tags").html('<div class="text-primary text-center"><small>There are currently no tags for this album. Log in to start adding your own tags!</small></div>'),0===$(".connection").length&&$("#connected-albums").html('<div class="text-primary text-center"><small>There are currently no connections for this album. Log in to start adding your own connections!</small><br/><br/></div>'))}
function addToFavorites(){if(albumResult.favorites&&albumResult.favorites.find(function(a){return a.userID===userID}))return populateUserLists(),alert('This album is already in your "My Favorites" list.');$.ajax("/api/v1/favorite",{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,album:albumResult}),success:function(a){$("#updateListModal").modal("hide");albumResult.favorites||(albumResult.favorites=[]);albumResult.favorites.push({userID:userID});populateListsWithAlbum();
updateListDisplay();$("#list-options").get(0).selectedIndex=0}})}
function removeFromFavorites(){confirm("Are you sure you want to remove this album from the 'My Favorites' list? You cannot undo this operation.")&&$.ajax("/api/v1/favorite",{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:userID,appleAlbumID:albumResult.appleAlbumID}),success:function(a){removeFromArray(albumResult.favorites,albumResult.favorites.find(function(a){return a.userID===userID}));populateListsWithAlbum();updateListDisplay()}})}
function getUserLists(){$.ajax({method:"GET",url:"/api/v1/list/user/"+userID,success:function(a){userLists=a.message?[]:a;populateUserLists()}})}
function populateListsWithAlbum(a){$("#all-lists").html("");$(".list-message").remove();userID&&albumResult.favorites&&albumResult.favorites.find(function(a){return a.userID===userID})&&$("#all-lists").append('<li class="list my-list" data-creator="'+userID+'"><a href="/list?type=myfavorites">My Favorites</a><span class="text-secondary"> by: You!</span><span class="remove-from-list-button" data-list-type="myfavorites">&#10005;</span></li>');albumResult.lists&&albumResult.lists.forEach(function(a){if(!a.isPrivate){var b=
a.displayName;b&&""!==b.trim||(b="Unknown");a.user===userID?$("#all-lists").append('<li class="list my-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary"> by: '+b+'</span><span class="remove-from-list-button" data-list-id="'+a.id+'" data-list-type="userlist">&#10005;</span></li>'):$("#all-lists").append('<li class="list other-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary" data-list-type="userlist"> by: '+
...
function populateUserLists(){$("#list-options").html("");$("<option selected>Add to a list...</option>").appendTo("#list-options");$("<option value='myfavorites'>My Favorites</option>").appendTo("#list-options");userLists.forEach(function(a){$('<option value="'+a.id+'">'+a.title+"</option>").appendTo("#list-options")})}
function addToList(a){if(a){if("myfavorites"===a)return addToFavorites();if(albumResult.lists){var b=albumResult.lists.find(function(b){return b.id===a});if(b)return $("#list-options").get(0).selectedIndex=0,alert('This album is already in your "'+b.title+'" list.')}$.ajax({method:"PUT",url:"/api/v1/list/"+a,contentType:"application/json",data:JSON.stringify({method:"add album",appleAlbumID:albumResult.appleAlbumID,title:albumResult.title,artist:albumResult.artist,releaseDate:albumResult.releaseDate,
cover:albumResult.cover,genres:albumResult.genres,songNames:albumResult.songNames,appleURL:albumResult.appleURL,recordCompany:albumResult.recordCompany}),success:function(a){albumResult.lists||(albumResult.lists=[]);albumResult.lists.push(a);populateListsWithAlbum();updateListDisplay();$("#updateListModal").modal("hide");$("#list-options").get(0).selectedIndex=0},error:function(a){console.log(a)}})}}
...
alert(a.message):(userLists.push(a),albumResult.lists||(albumResult.lists=[]),albumResult.lists.push(a),populateUserLists(),populateListsWithAlbum(),updateListDisplay(),$("#updateListModal").modal("hide"),$("#new-list-title").val(""),$("#new-display-name").val(""))}}))}
function removeFromList(a){var b=albumResult.lists.find(function(b){return b.id===a});confirm('Are you sure you want to remove this album from the "'+b.title+'" list? You cannot undo this operation.')&&$.ajax({method:"PUT",url:"/api/v1/list/"+b.id,contentType:"application/json",data:JSON.stringify({method:"remove album",appleAlbumID:albumResult.appleAlbumID,title:albumResult.title,artist:albumResult.artist,releaseDate:albumResult.releaseDate,cover:albumResult.cover}),success:function(a){a=albumResult.lists.indexOf(b);
albumResult.lists.splice(a,1);populateListsWithAlbum();updateListDisplay()}})}function updateListDisplay(){return"My Lists"===sessionStorage.getItem("lists")?displayMyLists():displayAllLists()}
...
function populateConnections(){if(albumResult.connections){$("#connected-albums").html("");for(var a=0;a<albumResult.connections.length;a++){var b=albumResult.connections[a];if(b.appleAlbumID!=albumResult.appleAlbumID){var c=b.cover.replace("{w}",105).replace("{h}",105),d=20<b.title.length?truncate(b.title,20):b.title;b.creator===userID?$("#connected-albums").append('<a href="/album/'+b.appleAlbumID+'" id="'+b.appleAlbumID+'" class="connection my-connection" data-creator="'+b.creator+'"><img class="connection-cover" src="'+
...
function addConnection(a){a&&a.title&&($.ajax("/api/v1/connection",{method:"POST",contentType:"application/json",data:JSON.stringify({albumOne:albumResult,albumTwo:a,creator:userID}),success:function(a){$("#updateConnectionModal").modal("hide");albumResult.connections=a;populateConnections();updateConnectionDisplay()}}),$("#add-connection-input").val(""))}var connectionSearchResults=[];
...
function deleteConnection(a){confirm("Are you sure you want to delete a connection? You cannot undo this operation.")&&$.ajax("/api/v1/connection",{method:"DELETE",contentType:"application/json",data:JSON.stringify({albumTwo:a.toString(),albumOne:albumResult.appleAlbumID.toString(),creator:userID}),success:function(a){albumResult.connections=a;populateConnections();updateConnectionDisplay()},error:function(a){if(a.responseJSON&&a.responseJSON.message)return alert(a.responseJSON.message)}})}
...
function populateTags(){clearTagArray();if(albumResult.tagObjects){albumResult.tagObjects=albumResult.tagObjects.sort(function(a,b){return a.text>b.text?1:-1});$("#current-tags").html("");for(var a=0;a<albumResult.tagObjects.length;a++){var b=albumResult.tagObjects[a].text,c=albumResult.tagObjects[a].creator,d=void 0;d=parseInt(b)?"tag_".concat(b).replace(/[^A-Z0-9]+/ig,""):b.replace(/[^A-Z0-9]+/ig,"");c===userID?$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag my-tag" data-creator="'+
c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].text)+'" data-custom-genre="'+(albumResult.tagObjects[a].customGenre||!1)+'"><span>'+b+'</span><span class="delete-tag-button ml-1" data-tag-id="'+d+"-"+c+'">&#10005;</span></a>'):$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag other-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].text)+'" data-custom-genre="'+(albumResult.tagObjects[a].customGenre||!1)+'"><span>'+b+"</span></a>")}$(".delete-tag-button").click(function(){deleteTag($(this).data("tag-id"))});
...
a=escapeHtml(a);if(albumResult.tagObjects){var b=0;albumResult.tagObjects.forEach(function(c){c.text===a&&c.creator===userID&&b++});if(0<b)return $("#add-tag-input").val(""),$("#custom-genre-checkbox").prop("checked",!1),alert('You already added the "'+a+'" tag to this album!');if(albumResult.tagObjects.find(function(b){return b.text===a})){var c=confirm('Someone else already added the "'+a+'" tag to this album. Choose "ok" to add your tag, or "cancel" to avoid duplicates.');$("#add-tag-input").val("");
$("#custom-genre-checkbox").prop("checked",!1);if(!c)return}}c=$("#custom-genre-checkbox").is(":checked");$.ajax("/api/v1/tag/",{method:"POST",contentType:"application/json",data:JSON.stringify({album:albumResult,creator:userID,tag:a,customGenre:c}),success:function(a){if(a.message)return alert(a.message);a.songNames&&a.genres||(a.songNames=albumResult.songNames,a.genres=albumResult.genres);albumResult=a;populateTags();updateTagDisplay();$("#tag-success").html("Added! &#10003;");setTimeout(function(){$("#tag-success").html("&nbsp;")},
