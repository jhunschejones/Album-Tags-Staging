...
function addToFavorites(){if(albumResult.favorited)return populateUserLists(),alert('This album is already in your "My Favorites" list.');$.ajax("/api/v1/favorite",{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,album:albumResult}),success:function(a){$("#updateListModal").modal("hide");albumResult.favorited=!0;populateListsWithAlbum();$("#list-options").get(0).selectedIndex=0}})}
function removeFromFavorites(){confirm("Are you sure you want to remove this album from the 'My Favorites' list? You cannot undo this operation.")&&$.ajax("/api/v1/favorite",{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:userID,appleAlbumID:albumResult.appleAlbumID}),success:function(a){albumResult.favorited=!1;populateListsWithAlbum()}})}
function getUserLists(){$.ajax({method:"GET",url:"/api/v1/list/user/"+userID,success:function(a){userLists=a.message?[]:a;populateUserLists()}})}function getListsWithAlbum(a){$.ajax({method:"GET",url:"/api/v1/list/album/"+albumResult.appleAlbumID,success:function(b){listsWithAlbum=b.message||""===b?[]:b;checkUserFavorites(a)}})}
function checkUserFavorites(a){userID?$.ajax({method:"GET",url:"/api/v1/favorite/"+userID,success:function(b){albumResult.favorited=!!b.find(function(a){return a.appleAlbumID===albumResult.appleAlbumID});populateListsWithAlbum(a)}}):populateListsWithAlbum(a)}
function populateListsWithAlbum(a){$("#all-lists").html("");$(".list-message").remove();userID&&albumResult.favorited&&$("#all-lists").append('<li class="list my-list" data-creator="'+userID+'"><a href="/list?type=myfavorites">My Favorites</a><span class="text-secondary"> by: You!</span><span class="remove-from-list-button" data-list-type="myfavorites">&#10005;</span></li>');listsWithAlbum.forEach(function(a){if(!a.isPrivate){var b=a.displayName;b&&""!==b.trim||(b="Unknown");a.user===userID?$("#all-lists").append('<li class="list my-list" data-creator="'+
a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary"> by: '+b+'</span><span class="remove-from-list-button" data-list-id="'+a.id+'" data-list-type="userlist">&#10005;</span></li>'):$("#all-lists").append('<li class="list other-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary" data-list-type="userlist"> by: '+b+"</span></li>")}});$(".remove-from-list-button").click(function(){if("myfavorites"===
$(this).data("list-type"))return removeFromFavorites();removeFromList($(this).data("list-id"))});if(a)return updateListDisplay();0===$(".list").length&&($(".list-message").remove(),$("#all-lists").after('<div class="text-primary text-center list-message"><small>This album is not in any public user lists yet. Log in to get started working with lists!</small><br/><br/></div>'))}
...
genres:albumResult.genres,songNames:albumResult.songNames,appleURL:albumResult.appleURL,recordCompany:albumResult.recordCompany}),success:function(a){listsWithAlbum.push(a);populateListsWithAlbum();$("#updateListModal").modal("hide");$("#list-options").get(0).selectedIndex=0},error:function(a){console.log(a)}})}}
...
listsWithAlbum.splice(a,1);populateListsWithAlbum()}})}function updateListDisplay(){return"My Lists"===sessionStorage.getItem("lists")?displayMyLists():displayAllLists()}
...
$("#show-my-lists").click(function(){sessionStorage.setItem("lists","All Lists");updateListDisplay()})}function getConnections(){$.ajax("/api/v1/connection/"+albumResult.appleAlbumID,{method:"GET",contentType:"application/json",success:function(a){albumResult.connectionObjects="This user has not created any connections for this album."===a.message?[]:a;populateConnections();updateConnectionDisplay()}})}
...
'"><img class="connection-cover" src="'+c+'" data-toggle="tooltip" data-placement="top" title="'+d+'" data-trigger="hover"><span class="delete-connection-button" data-connected-album-id="'+b.appleAlbumID+'">&#10005;</span></a>'):$("#connected-albums").append('<a href="/album/'+b.appleAlbumID+'" id="'+b.appleAlbumID+'" class="connection other-connection" data-creator="'+b.creator+'"><img class="connection-cover" src="'+c+'" data-toggle="tooltip" data-placement="top" title="'+d+'" data-trigger="hover"></a>')}}$(".delete-connection-button").click(function(a){a.preventDefault();
deleteConnection($(this).data("connected-album-id"))});a=!1;"ontouchstart"in document.documentElement&&(a=!0);a||$('[data-toggle="tooltip"]').tooltip()}}function addConnection(a){a&&a.title&&($.ajax("/api/v1/connection",{method:"POST",contentType:"application/json",data:JSON.stringify({albumOne:albumResult,albumTwo:a,creator:userID}),success:function(a){$("#updateConnectionModal").modal("hide");getConnections()}}),$("#add-connection-input").val(""))}var connectionSearchResults=[];
...
function deleteConnection(a){confirm("Are you sure you want to delete a connection? You cannot undo this operation.")&&$.ajax("/api/v1/connection",{method:"DELETE",contentType:"application/json",data:JSON.stringify({albumTwo:a.toString(),albumOne:albumResult.appleAlbumID.toString(),creator:userID}),success:function(a){getConnections();updateConnectionDisplay()},error:function(a){if(a.responseJSON&&a.responseJSON.message)return alert(a.responseJSON.message)}})}
...
function deleteTag(a){var b=$("#"+a).data("creator");a=$("#"+a).data("rawtag");confirm('Are you sure you want to delete the "'+a+'" tag? You cannot undo this operation.')&&$.ajax("/api/v1/tag/",{method:"DELETE",contentType:"application/json",data:JSON.stringify({text:escapeHtml(a.toString()),creator:b,appleAlbumID:albumResult.appleAlbumID}),success:function(a){if(a.message)alert(a.message);else{if(1===a.deleted){var b=albumResult.tagObjects.find(function(b){return b.text===a.tag.text&&b.creator===
a.tag.creator});removeFromArray(albumResult.tagObjects,b)}populateTags();updateTagDisplay()}}})}
