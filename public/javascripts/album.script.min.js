...
$(".list").length&&$("#all-lists").after('<div class="text-primary text-center list-message"><small>This album is not in any public user lists. Click "Add to a list" below to get started!</small><br/><br/></div>');$("#show-all-lists").click(function(a){a.preventDefault();sessionStorage.setItem("lists","My Lists");updateListDisplay()})}
...
$("#show-my-lists").click(function(a){a.preventDefault();sessionStorage.setItem("lists","All Lists");updateListDisplay()})}
...
function addConnection(a){$.getJSON("/api/v1/album/albumid/"+a.appleAlbumID,function(b){b.message?"No matching album in the database."===b.message?$.ajax("/api/v1/album/connections/"+(albumResult._id||"new"),{method:"POST",contentType:"application/json",data:JSON.stringify({albumOne:albumResult,albumTwo:a,creator:userID}),success:function(a){a.message?alert(a.message):(albumResult=a,populateConnections(),$("#updateConnectionModal").modal("hide"))}}):alert(b.message):$.ajax("/api/v1/album/connections/"+
(albumResult._id||"new"),{method:"POST",contentType:"application/json",data:JSON.stringify({albumOne:albumResult,albumTwo:b,creator:userID}),success:function(a){a.message?alert(a.message):(albumResult=a,populateConnections(),$("#updateConnectionModal").modal("hide"))}})});$("#add-connection-input").val("")}var connectionSearchResults=[];
function populateConnectionModalResults(a){$("#connection-search-results").html("");$("#connection-loader").hide();if(a.albums){for(var b=0;b<a.albums.length;b++){var c=a.albums[b],d=b+1;connectionSearchResults.push(c);createConnectionModalCard(c,d);populateConnectionModalCard(c,d)}$("#connection-search-results").append('<div id="connection-search-modal-placeholder">&nbsp;</div>')}}
...
$("#no-other-connections").remove();0===$(".connection").length&&$("#connected-albums").html('<div id="no-other-connections" class="text-primary text-center"><small>There are currently no connections for this album. Click "Add connections" below to get started!</small><br/><br/></div>');$("#show-all-connections").click(function(a){a.preventDefault();sessionStorage.setItem("connections","My Connections");updateConnectionDisplay()})}
...
$("#no-other-connections").remove();0===$(".my-connection").length&&$("#connected-albums").append('<div id="no-my-connections" class="text-primary text-center"><small>You have not created any connections for this album. Click "Add connections" below to get started!</small><br/><br/></div>');$("#show-my-connections").click(function(a){a.preventDefault();sessionStorage.setItem("connections","All Connections");updateConnectionDisplay()})}
function populateTags(){if(albumResult.tagObjects){$("#current-tags").html("");for(var a=0;a<albumResult.tagObjects.length;a++){var b=albumResult.tagObjects[a].tag,c=albumResult.tagObjects[a].creator,d=void 0;d=parseInt(b)?"tag_".concat(b).replace(/[^A-Z0-9]+/ig,""):b.replace(/[^A-Z0-9]+/ig,"");c===userID?$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag my-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].tag)+'"><span>'+b+'</span><span class="delete-tag-button ml-1" data-tag-id="'+
d+"-"+c+'">&#10005;</span></a>'):$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag other-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].tag)+'"><span>'+b+"</span></a>")}$(".delete-tag-button").click(function(a){a.preventDefault();deleteTag($(this).data("tag-id"))});$(".album-tag").click(function(a){a.preventDefault();selectTag(document.getElementById($(this).attr("id")),a)})}}
...
{tag:a,creator:userID})&&b++});if(0<b){alert('You already added the "'+a+'" tag to this album!');$("#add-tag-input").val("");return}if(-1!=albumResult.tags.indexOf(a)){var c=confirm('Someone else already added the "'+a+'" tag to this album. Choose "ok" to add your tag, or "cancel" to avoid duplicates.');$("#add-tag-input").val("");if(!c)return}}$.ajax("/api/v1/album/tags/"+(albumResult._id||"new"),{method:"POST",contentType:"application/json",data:JSON.stringify({album:albumResult,creator:userID,
tag:a}),success:function(a){a.message?alert(a.message):(albumResult=a,populateTags(),$("#tag-success").text("You successfully added a tag!"),setTimeout(function(){$("#tag-success").html("&nbsp;")},3E3))}})}else alert("Please enter a non-empty tag.");$("#add-tag-input").val("")}
function toggleActiveInfoTab(a){$("#info-card .active").removeClass("active").addClass("inactive-info-tab");$(a).removeClass("inactive-info-tab").addClass("active");$(".info-card-body").hide();a=a.data("card");$("#"+a+"-info-card-body").show()}function clearTagArray(a){a&&a.preventDefault();0<$(".selected-tag").length&&($(".selected-tag").toggleClass("badge-primary"),$(".selected-tag").toggleClass("badge-light"),$(".selected-tag").toggleClass("selected-tag"),selectedTags=[])}
function updateTagDisplay(a){var b=!1;if(a||userID)b=!0;"My Tags"===sessionStorage.getItem("tags")&&b?displayMyTags(b):displayAllTags(b)}
...
$("#current-tags").append('<div id="no-all-tags" class="text-primary text-center"><small>There are currently no tags for this album. Click "Add tags" below to get started!</small></div>');a&&$("#show-all-tags").click(function(b){b.preventDefault();sessionStorage.setItem("tags","My Tags");updateTagDisplay(a)})}
...
$("#show-my-tags").click(function(b){b.preventDefault();sessionStorage.setItem("tags","All Tags");updateTagDisplay(a)})}$("#favorites-link").click(function(a){a.preventDefault();a=window.open(window.location.protocol+"//"+window.location.host+"/myfavorites","_blank");a.location});$("#new-list-title").keyup(function(a){13===a.keyCode&&$("#add-to-new-list-button").click()});$("#new-display-name").keyup(function(a){13===a.keyCode&&$("#add-to-new-list-button").click()});
$("#list-options").keyup(function(a){13===a.keyCode&&$("#add-to-list-button").click()});$("#search-for-second-album").click(function(){var a=window.open(window.location.protocol+"//"+window.location.host+"/search","_blank");a.location});$("#info-card .nav-link").click(function(a){a.preventDefault();toggleActiveInfoTab($(this))});$("#tag-search-button").click(function(a){a.preventDefault();0<selectedTags.length?window.location="/search/tags/"+selectedTags:alert("Select one or more tags to preform a tag search")});
$("#clear-tag-button").click(function(a){a.preventDefault();clearTagArray(a)});$("#add-tag-button").click(function(a){a.preventDefault();addTag()});$("#add-tag-input").keyup(function(a){13===a.keyCode&&$("#add-tag-button").click()});$("#add-connection-button").click(function(a){a.preventDefault();a=$("#add-connection-input").val().trim();$("#connection-search-results").html("");$("#connection-loader").show();executeSearch(a,"connection")});
$("#add-connection-input").keyup(function(a){13===a.keyCode&&$("#add-connection-button").click()});$("#add-to-list-button").click(function(a){a.preventDefault();a=document.getElementById("list-options");a=a[a.selectedIndex].value;"Add to a list..."!==a&&addToList(a)});
...
$(document).ready(function(){var a=!1;"ontouchstart"in document.documentElement&&(a=!0);a&&(a=document.getElementById("connection-search-results"),a.style.paddingBottom="0px",a.style.overflowX="scroll")});
