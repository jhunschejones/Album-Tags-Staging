...
function getAlbumDetails(a){$.ajax("/api/v1/album/"+albumID,{method:"GET",success:function(b){b.message?$.getJSON("/api/v1/apple/details/"+albumID,function(b){b.message?alert(b.message):(albumResult=b,populateAlbumPage(a))}):(albumResult=b,populateAlbumPage(a))}})}
...
function addToFavorites(){if(albumResult._id&&-1!==albumResult.favoritedBy.indexOf(userID))return populateUserLists(),alert('This album is already in your "My Favorites" list.');$.ajax("/api/v1/album/favorites/"+(albumResult._id||"new"),{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,albumData:albumResult}),success:function(a){a.message?alert(a.message):(albumResult=a,populateListsWithAlbum(),$("#updateListModal").modal("hide"),$("#list-options").get(0).selectedIndex=
0)}})}function removeFromFavorites(){confirm("Are you sure you want to remove this album from the 'My Favorites' list? You cannot undo this operation.")&&$.ajax("/api/v1/album/favorites/"+albumResult._id,{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:userID}),success:function(a){a.message?alert(a.message):(albumResult=a,populateListsWithAlbum())}})}
...
function populateListsWithAlbum(a){$("#all-lists").html("");$(".list-message").remove();albumResult._id&&userID&&-1!==albumResult.favoritedBy.indexOf(userID)&&$("#all-lists").append('<li class="list my-list" data-creator="'+userID+'"><a href="/list?type=myfavorites">My Favorites</a><span class="text-secondary"> by: You!</span><span class="remove-from-list-button" data-list-type="myfavorites">&#10005;</span></li>');listsWithAlbum.forEach(function(a){if(!a.isPrivate){var b=a.displayName;""===b.trim&&
(b="Unknown");a.user===userID?$("#all-lists").append('<li class="list my-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a._id+'">'+a.title+'</a><span class="text-secondary"> by: '+b+'</span><span class="remove-from-list-button" data-list-id="'+a._id+'" data-list-type="userlist">&#10005;</span></li>'):$("#all-lists").append('<li class="list other-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a._id+'">'+a.title+'</a><span class="text-secondary" data-list-type="userlist"> by: '+
...
function populateUserLists(){$("#list-options").html("");$("<option selected>Add to a list...</option>").appendTo("#list-options");$("<option value='myfavorites'>My Favorites</option>").appendTo("#list-options");userLists.forEach(function(a){$('<option value="'+a._id+'">'+a.title+"</option>").appendTo("#list-options")})}
function addToList(a){if(a){if("myfavorites"===a)return addToFavorites();var b=listsWithAlbum.find(function(b){return b._id===a});if(b)return $("#list-options").get(0).selectedIndex=0,alert('This album is already in your "'+b.title+'" list.');$.ajax({method:"PUT",url:"/api/v1/list/"+a,contentType:"application/json",data:JSON.stringify({method:"add album",appleAlbumID:albumResult.appleAlbumID,title:albumResult.title,artist:albumResult.artist,releaseDate:albumResult.releaseDate,cover:albumResult.cover,
genres:albumResult.genres}),success:function(a){a.message?alert(a.message):(listsWithAlbum.push(a),populateListsWithAlbum(),$("#updateListModal").modal("hide"),$("#list-options").get(0).selectedIndex=0)}})}}
