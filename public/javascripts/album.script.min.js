...
function addToFavorites(){if(albumResult.favorites.find(function(a){return a.userID===userID}))return populateUserLists(),alert('This album is already in your "My Favorites" list.');$.ajax("/api/v1/favorite",{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,album:albumResult}),success:function(a){$("#updateListModal").modal("hide");albumResult.favorites.push({userID:userID});populateListsWithAlbum();updateListDisplay();$("#list-options").get(0).selectedIndex=0}})}
...
alert(a.message):(userLists.push(a),albumResult.lists.push(a),populateUserLists(),populateListsWithAlbum(),updateListDisplay(),$("#updateListModal").modal("hide"),$("#new-list-title").val(""),$("#new-display-name").val(""))}}))}
