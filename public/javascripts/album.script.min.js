...
this.onSettledCallbacks_=null}};var f=new b;e.prototype.settleSameAsPromise_=function(a){var b=this.createResolveAndReject_();a.callWhenSettled_(b.resolve,b.reject)};e.prototype.settleSameAsThenable_=function(a,b){var c=this.createResolveAndReject_();try{a.call(b,c.resolve,c.reject)}catch(k){c.reject(k)}};e.prototype.then=function(a,b){function c(a,b){return"function"==typeof a?function(b){try{d(a(b))}catch(l){f(l)}}:b}var d,f,h=new e(function(a,b){d=a;f=b});this.callWhenSettled_(c(a,d),c(b,f));return h};
...
function makeNiceDate(a){var b=a.slice(0,4),c=a.slice(8,10);return"January February March April May June July August September October November December".split(" ")[a.slice(5,7)-1]+" "+c+", "+b}
function removeDOMElement(a){if("."===a[0]){if((a=document.getElementsByClassName(a.substring(1)))&&0<a.length)for(var b=0;b<a.length;b++)a[b].parentNode.removeChild(a[b])}else"#"===a[0]?(a=document.getElementById(a.substring(1)))&&0<a.length&&a.parentNode.removeChild(a):console.error("removeDOMElement() was passed an invalid selector")}function addClassEventListener(a,b,c){a=document.getElementsByClassName(a);for(var d=0;d<a.length;d++)a[d].addEventListener(b,c)}
function htmlToElement(a){var b=document.createElement("template");b.innerHTML=a;return b.content.firstChild}function showClass(a){a=document.getElementsByClassName(a);for(var b=0;b<a.length;b++)a[b].classList.remove("hide_me")}function hideClass(a){a=document.getElementsByClassName(a);for(var b=0;b<a.length;b++)a[b].classList.add("hide_me")}
...
function getAlbumDetails(a){var b,c,d,e;return $jscomp.asyncExecutePromiseGeneratorProgram(function(f){switch(f.nextAddress){case 1:return f.yield(fetch("/api/v1/album/"+albumID),2);case 2:b=f.yieldResult;if(!b.ok)throw Error(b.statusText);return f.yield(b.json(),3);case 3:c=f.yieldResult;if(!c.message||"No album found"!==c.message.slice(0,14)){app.album=c;populateAlbumPage(a);f.jumpTo(0);break}return f.yield(fetch("/api/v1/apple/details/"+albumID),5);case 5:d=f.yieldResult;if(!d.ok)throw Error(d.statusText);
return f.yield(d.json(),6);case 6:e=f.yieldResult;if(e.message)return f.return(alert(e.message));app.album=e;populateAlbumPage(a);f.jumpToEnd()}})}
function populateAlbumPage(a){populateTags();populateConnections();getUserLists();a?(updateTagDisplay(a),updateConnectionDisplay(a),populateListsWithAlbum(a)):(0===document.getElementsByClassName("album-tag").length&&document.getElementById("current-tags").appendChild(htmlToElement('<div class="text-primary text-center"><small>There are currently no tags for this album. Log in to start adding your own tags!</small></div>')),0===document.getElementsByClassName("connection").length&&document.getElementById("connected-albums").appendChild(htmlToElement('<div class="text-primary text-center"><small>There are currently no connections for this album. Log in to start adding your own connections!</small><br/><br/></div>')))}
function addToFavorites(){if(app.album.favorites&&app.album.favorites.find(function(a){return a.userID===app.userID}))return populateUserLists(),alert('This album is already in your "My Favorites" list.');$.ajax("/api/v1/favorite",{method:"POST",contentType:"application/json",data:JSON.stringify({user:app.userID,album:app.album}),success:function(a){(new Modal(document.getElementById("updateListModal"))).hide();app.album.favorites||(app.album.favorites=[]);app.album.favorites.push({userID:app.userID});
populateListsWithAlbum();updateListDisplay();document.getElementById("list-options").selectedIndex=0}})}function removeFromFavorites(){confirm("Are you sure you want to remove this album from the 'My Favorites' list? You cannot undo this operation.")&&$.ajax("/api/v1/favorite",{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:app.userID,appleAlbumID:app.album.appleAlbumID,returnData:"album"}),success:function(a){app.album=a;populateListsWithAlbum();updateListDisplay()}})}
function getUserLists(){$.ajax({method:"GET",url:"/api/v1/list/user/"+app.userID,success:function(a){app.userLists=a.message?[]:a;populateUserLists()}})}
function populateListsWithAlbum(a){document.getElementById("all-lists").innerHTML="";removeDOMElement(".list-message");app.userID&&app.album.favorites&&app.album.favorites.find(function(a){return a.userID===app.userID})&&document.getElementById("all-lists").appendChild(htmlToElement('<li class="list my-list" data-creator="'+app.userID+'"><a href="/list?type=myfavorites">&#9825; My Favorites</a><span class="text-secondary"> by: You!</span><span class="remove-from-list-button" data-list-type="myfavorites">&#10005;</span></li>'));app.album.lists&&
(app.album.lists=app.album.lists.sort(function(a,c){return a.title>c.title?1:-1}),app.album.lists.forEach(function(a){if(!a.isPrivate){var b=a.displayName;b&&""!==b.trim||(b="Unknown");a.user===app.userID?document.getElementById("all-lists").appendChild(htmlToElement('<li class="list my-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary"> by: '+b+'</span><span class="remove-from-list-button" data-list-id="'+a.id+'" data-list-type="userlist">&#10005;</span></li>')):
document.getElementById("all-lists").appendChild(htmlToElement('<li class="list other-list" data-creator="'+a.user+'"><a href="/list?type=userlist&id='+a.id+'">'+a.title+'</a><span class="text-secondary" data-list-type="userlist"> by: '+b+"</span></li>"))}}));addClassEventListener("remove-from-list-button","click",function(){if("myfavorites"===this.dataset.listType)return removeFromFavorites();removeFromList(parseInt(this.dataset.listId))});if(a)return updateListDisplay();0===document.getElementsByClassName("list").length&&
(removeDOMElement(".list-message"),document.getElementById("all-lists").appendChild(htmlToElement('<div class="text-primary text-center list-message"><small>This album is not in any public user lists yet. Log in to get started working with lists!</small><br/><br/></div>')))}
function populateUserLists(){document.getElementById("list-options").innerHTML="";document.getElementById("list-options").appendChild(htmlToElement("<option selected>Add to a list...</option>"));document.getElementById("list-options").appendChild(htmlToElement('<option value="myfavorites">&#9825; My Favorites</option>'));app.userLists=app.userLists.sort(function(a,b){return a.title>b.title?1:-1});app.userLists.forEach(function(a){document.getElementById("list-options").appendChild(htmlToElement('<option value="'+
a.id+'">'+a.title+"</option>"))})}
function addToList(a){if(a){if("myfavorites"===a)return addToFavorites();if(app.album.lists){var b=app.album.lists.find(function(b){return b.id===a});if(b)return document.getElementById("list-options").selectedIndex=0,alert('This album is already in your "'+b.title+'" list.')}$.ajax({method:"PUT",url:"/api/v1/list/"+a,contentType:"application/json",data:JSON.stringify({method:"add album",appleAlbumID:app.album.appleAlbumID,title:app.album.title,artist:app.album.artist,releaseDate:app.album.releaseDate,
cover:app.album.cover,genres:app.album.genres,songNames:app.album.songNames,appleURL:app.album.appleURL,recordCompany:app.album.recordCompany}),success:function(a){app.album.lists||(app.album.lists=[]);app.album.lists.push(a);populateListsWithAlbum();updateListDisplay();(new Modal(document.getElementById("updateListModal"))).hide();document.getElementById("list-options").selectedIndex=0},error:function(a){console.log(a)}})}}
function addToNewList(a,b){var c=!0,d=app.userLists.find(function(b){return b.title.toUpperCase()===a.toUpperCase()});d&&(c=confirm('You already have a list called "'+d.title+'". Choose "ok" to create a new list, "cancel" to go back and add this album to an existing list.'));c&&(c=!1,document.getElementById("private-checkbox")&&document.getElementById("private-checkbox").checked&&(c=!0),a&&b&&$.ajax({method:"POST",url:"/api/v1/list/",contentType:"application/json",data:JSON.stringify({user:app.userID,
displayName:b,title:a,isPrivate:c,albums:[app.album]}),success:function(a){a.message?alert(a.message):(app.userLists.push(a),app.album.lists||(app.album.lists=[]),app.album.lists.push(a),populateUserLists(),populateListsWithAlbum(),updateListDisplay(),(new Modal(document.getElementById("updateListModal"))).hide(),document.getElementById("new-list-title").value="",document.getElementById("new-display-name").value="")}}))}
function removeFromList(a){var b=app.album.lists.find(function(b){return b.id===a});confirm('Are you sure you want to remove this album from the "'+b.title+'" list? You cannot undo this operation.')&&$.ajax({method:"PUT",url:"/api/v1/list/"+b.id,contentType:"application/json",data:JSON.stringify({method:"remove album",appleAlbumID:app.album.appleAlbumID,title:app.album.title,artist:app.album.artist,releaseDate:app.album.releaseDate,cover:app.album.cover}),success:function(a){a=app.album.lists.indexOf(b);
app.album.lists.splice(a,1);populateListsWithAlbum();updateListDisplay()}})}function updateListDisplay(){return"My Lists"===sessionStorage.getItem("lists")?displayMyLists():displayAllLists()}
...
for(var a=document.getElementsByClassName("my-list"),b=0;b<a.length;b++)a[b].classList.remove("hide_me");a=document.getElementsByClassName("other-list");for(b=0;b<a.length;b++)a[b].classList.remove("hide_me");document.getElementById("list-title-modifier").innerHTML='All <span class="large-button-text">user </span>';removeDOMElement(".list-message");0===document.getElementsByClassName("list").length&&document.getElementById("all-lists").parentNode.insertBefore(htmlToElement('<div class="text-primary text-center list-message"><small>This album is not in any public user lists. Click "Add to a list" below to get started!</small><br/><br/></div>'),
document.getElementById("all-lists").nextSibling);document.getElementById("show-all-lists").addEventListener("click",function(){sessionStorage.setItem("lists","My Lists");updateListDisplay()})}
...
for(var a=document.getElementsByClassName("my-list"),b=0;b<a.length;b++)a[b].classList.remove("hide_me");a=document.getElementsByClassName("other-list");for(b=0;b<a.length;b++)a[b].classList.add("hide_me");document.getElementById("list-title-modifier").innerText="Your ";removeDOMElement(".list-message");0===document.getElementsByClassName("my-list").length&&document.getElementById("all-lists").parentNode.insertBefore(htmlToElement('<div class="text-primary text-center list-message"><small>You have not added this album to any lists. Click "Add to a list" below to get started!</small><br/><br/></div>'),
...
function populateConnections(){if(app.album.connections){document.getElementById("connected-albums").innerHTML="";for(var a=0;a<app.album.connections.length;a++){var b=app.album.connections[a];if(b.appleAlbumID!=app.album.appleAlbumID){var c=b.cover.replace("{w}",105).replace("{h}",105),d=20<b.title.length?truncate(b.title,20):b.title;b.creator===app.userID?document.getElementById("connected-albums").appendChild(htmlToElement('<a href="/album/'+b.appleAlbumID+'" id="'+b.appleAlbumID+'" class="connection my-connection" data-creator="'+
b.creator+'"><img class="connection-cover" src="'+c+'" data-toggle="tooltip" data-placement="top" title="'+d+'" data-trigger="hover"><span class="delete-connection-button" data-connected-album-id="'+b.appleAlbumID+'">&#10005;</span></a>')):document.getElementById("connected-albums").appendChild(htmlToElement('<a href="/album/'+b.appleAlbumID+'" id="'+b.appleAlbumID+'" class="connection other-connection" data-creator="'+b.creator+'"><img class="connection-cover" src="'+c+'" data-toggle="tooltip" data-placement="top" title="'+
d+'" data-trigger="hover"></a>'))}}addClassEventListener("delete-connection-button","click",function(a){a.preventDefault();deleteConnection(this.dataset.connectedAlbumId)});a=!1;"ontouchstart"in document.documentElement&&(a=!0);if(!a)for(a=document.querySelectorAll("[title]"),b=0;b<a.length;b++)new Tooltip(a[b],{placement:"top",animation:"hide",delay:10})}}
function addConnection(a){a&&a.title&&($.ajax("/api/v1/connection",{method:"POST",contentType:"application/json",data:JSON.stringify({albumOne:app.album,albumTwo:a,creator:app.userID}),success:function(a){(new Modal(document.getElementById("updateConnectionModal"))).hide();app.album.connections=a;populateConnections();updateConnectionDisplay()}}),document.getElementById("add-connection-input").value="")}
function populateConnectionModalResults(a){document.getElementById("connection-search-results").innerHTML="";document.getElementById("connection-loader").classList.add("hide_me");if(a.albums&&0<a.albums.length){for(var b=0;b<a.albums.length;b++){var c=a.albums[b],d=b+1;app.connectionSearchResults.push(c);createConnectionModalCard(c,d);populateConnectionModalCard(c,d)}document.getElementById("connection-search-results").appendChild(htmlToElement('<div id="connection-search-modal-placeholder">&nbsp;</div>'))}else document.getElementById("connection-search-results").parentNode.insertBefore(htmlToElement('<div id="no-results-message" class="text-primary mb-3" style="text-align:center;">It looks like no albums matched your search terms. Try a different search!</div>'),
document.getElementById("connection-search-results").nextSibling)}
function createConnectionModalCard(a,b){document.getElementById("connection-search-results").appendChild(htmlToElement('<div id="connectionModalCard'+b+'" class="search-modal-card" data-apple-album-id="'+a.appleAlbumID+'"><a class="search-modal-card-album-link" href=""><img class="search-modal-card-image" src="" alt=""><a/><div class="search-modal-card-body"><h4 class="search-modal-card-title"></h4><span class="search-modal-card-album"></span></div></div>'))}
function populateConnectionModalCard(a,b){var c=32<a.artist.length?truncate(a.artist,32):a.artist,d=49<a.artist.length?truncate(a.artist,49):a.artist,e=44<a.title.length?truncate(a.title,44):a.title,f=66<a.title.length?truncate(a.title,66):a.title;document.querySelector("#connectionModalCard"+b+" .search-modal-card-title").innerHTML='<span class="search-modal-card-large-artist">'+d+'</span><span class="search-modal-card-small-artist">'+c+"</span>";document.querySelector("#connectionModalCard"+b+" .search-modal-card-album").innerHTML=
'<span class="search-modal-card-large-album">'+f+'</span><span class="search-modal-card-small-album">'+e+"</span>";document.querySelector("#connectionModalCard"+b+" .search-modal-card-image").src=a.cover.replace("{w}",260).replace("{h}",260);document.getElementById("connectionModalCard"+b).addEventListener("click",function(a){a.preventDefault();var b=this.dataset.appleAlbumId;a=app.connectionSearchResults.find(function(a){return a.appleAlbumID===b.toString()});addConnection(a)})}
function deleteConnection(a){confirm("Are you sure you want to delete a connection? You cannot undo this operation.")&&$.ajax("/api/v1/connection",{method:"DELETE",contentType:"application/json",data:JSON.stringify({albumTwo:a.toString(),albumOne:app.album.appleAlbumID.toString(),creator:app.userID}),success:function(a){app.album.connections=a;populateConnections();updateConnectionDisplay()},error:function(a){if(a.responseJSON&&a.responseJSON.message)return alert(a.responseJSON.message)}})}
...
showClass("my-connection");hideClass("other-connection");document.getElementById("connection-title-modifier").innerText="you";removeDOMElement(".no-my-connections");removeDOMElement(".no-other-connections");0===document.getElementsByClassName("my-connection").length&&document.getElementById("connected-albums").appendChild(htmlToElement('<div class="no-my-connections text-primary text-center"><small>You have not created any connections for this album. Click "Add connections" below to get started!</small><br/><br/></div>'));
...
function populateTags(){clearTagArray();if(app.album.tagObjects){app.album.tagObjects=app.album.tagObjects.sort(function(a,b){return a.text>b.text?1:-1});document.getElementById("current-tags").innerHTML="";for(var a=0;a<app.album.tagObjects.length;a++){var b=app.album.tagObjects[a].text,c=app.album.tagObjects[a].creator,d=void 0;d=parseInt(b)?"tag_".concat(b).replace(/[^A-Z0-9]+/ig,""):b.replace(/[^A-Z0-9]+/ig,"");c===app.userID?document.getElementById("current-tags").appendChild(htmlToElement('<a href="" id="'+
d+"-"+c+'" class="badge badge-light album-tag my-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(app.album.tagObjects[a].text)+'" data-custom-genre="'+(app.album.tagObjects[a].customGenre||!1)+'"><span>'+b+'</span><span class="delete-tag-button ml-1" data-tag-id="'+d+"-"+c+'">&#10005;</span></a>')):document.getElementById("current-tags").appendChild(htmlToElement('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag other-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(app.album.tagObjects[a].text)+
...
function deleteTag(a){var b=document.getElementById(""+a).dataset.creator,c=document.getElementById(""+a).dataset.customGenre;a=document.getElementById(""+a).dataset.rawtag;confirm('Are you sure you want to delete the "'+a+'" tag? You cannot undo this operation.')&&$.ajax("/api/v1/tag/",{method:"DELETE",contentType:"application/json",data:JSON.stringify({text:escapeHtml(a.toString()),creator:b,appleAlbumID:app.album.appleAlbumID,customGenre:c}),success:function(a){a.message?alert(a.message):(app.album=
a,populateTags(),updateTagDisplay())}})}
function addTag(){var a=document.getElementById("add-tag-input").value.trim();if(a){if(a.includes("<")&&a.includes(">")||a.includes(".")||a.includes("{")||a.includes("}")){alert("Some characters are not allowed in tags, sorry!");document.getElementById("add-tag-input").value="";document.getElementById("custom-genre-checkbox").checked=!1;return}if(30<a.length){alert('Tags cannot be longer than 30 characters. Check out the "All tags" page for some examples!');document.getElementById("add-tag-input").value=
"";document.getElementById("custom-genre-checkbox").checked=!1;return}a=removeExtraSpaces(toTitleCase(a));a=escapeHtml(a);if(app.album.tagObjects){var b=0;app.album.tagObjects.forEach(function(c){c.text===a&&c.creator===app.userID&&b++});if(0<b)return document.getElementById("add-tag-input").value="",document.getElementById("custom-genre-checkbox").checked=!1,alert('You already added the "'+a+'" tag to this album!');if(app.album.tagObjects.find(function(b){return b.text===a})){var c=confirm('Someone else already added the "'+
a+'" tag to this album. Choose "ok" to add your tag, or "cancel" to avoid duplicates.');document.getElementById("add-tag-input").value="";document.getElementById("custom-genre-checkbox").checked=!1;if(!c)return}}c=document.getElementById("custom-genre-checkbox").checked;$.ajax("/api/v1/tag/",{method:"POST",contentType:"application/json",data:JSON.stringify({album:app.album,creator:app.userID,tag:a,customGenre:c}),success:function(a){if(a.message)return alert(a.message);a.songNames&&a.genres||(a.songNames=
app.album.songNames,a.genres=app.album.genres);app.album=a;populateTags();updateTagDisplay();document.getElementById("tag-success").innerHTML="Added! &#10003;";setTimeout(function(){document.getElementById("tag-success").innerHTML="&nbsp;"},3E3)},error:function(a){console.log(a)}})}else alert("Please enter a non-empty tag.");document.getElementById("add-tag-input").value="";document.getElementById("custom-genre-checkbox").checked=!1}
...
hideClass("other-tag");document.getElementById("tag-title-modifier").innerText="you";removeDOMElement(".no-all-tags");removeDOMElement(".no-my-tags");0===document.getElementsByClassName("my-tag").length&&document.getElementById("current-tags").appendChild(htmlToElement('<div class="no-my-tags text-primary text-center"><small>You have not created any tags for this album. Click "Add tags" below to get started!</small></div>'));document.getElementById("show-my-tags").addEventListener("click",function(){sessionStorage.setItem("tags",
"All Tags");updateTagDisplay(a)})}document.getElementById("tag-update-button").addEventListener("click",function(){(new Modal(document.getElementById("updateTagModal"))).show()});document.getElementById("connection-update-button").addEventListener("click",function(){(new Modal(document.getElementById("updateConnectionModal"))).show()});document.getElementById("list-update-button").addEventListener("click",function(){(new Modal(document.getElementById("updateListModal"))).show()});
...
document.getElementById("add-connection-button").addEventListener("click",function(){var a=document.getElementById("add-connection-input").value.trim();removeDOMElement("#no-results-message");document.getElementById("connection-search-results").innerHTML="";document.getElementById("connection-loader").classList.remove("hide_me");executeSearch(a,"connection")});document.getElementById("add-connection-input").addEventListener("keyup",function(a){13===a.keyCode&&document.getElementById("add-connection-button").click()});
