...
function populateTags(){clearTagArray();if(albumResult.tagObjects){$("#current-tags").html("");for(var a=0;a<albumResult.tagObjects.length;a++){var b=albumResult.tagObjects[a].tag,c=albumResult.tagObjects[a].creator,d=void 0;d=parseInt(b)?"tag_".concat(b).replace(/[^A-Z0-9]+/ig,""):b.replace(/[^A-Z0-9]+/ig,"");c===userID?$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag my-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].tag)+'"><span>'+
b+'</span><span class="delete-tag-button ml-1" data-tag-id="'+d+"-"+c+'">&#10005;</span></a>'):$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag other-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].tag)+'"><span>'+b+"</span></a>")}$(".delete-tag-button").click(function(a){a.preventDefault();deleteTag($(this).data("tag-id"))});$(".album-tag").click(function(a){a.preventDefault();selectTag(document.getElementById($(this).attr("id")),
a)})}}function selectTag(a,b){b&&b.preventDefault();a=document.getElementById(a.id);a.classList.toggle("badge-primary");a.classList.toggle("selected-tag");a.classList.toggle("badge-light");modifySelectedTags(escapeHtml(a.dataset.rawtag))}function modifySelectedTags(a){-1===$.inArray(a,selectedTags)?selectedTags.push(a):(a=selectedTags.indexOf(a),selectedTags.splice(a,1))}
...
$("#clear-tag-button").click(function(a){a.preventDefault();clearTagArray(a)});$("#add-tag-button").click(function(a){a.preventDefault();addTag()});$("#add-tag-input").keyup(function(a){13===a.keyCode&&$("#add-tag-button").click()});$("#add-connection-button").click(function(a){a.preventDefault();a=$("#add-connection-input").val().trim();$("#no-results-message").remove();$("#connection-search-results").html("");$("#connection-loader").show();executeSearch(a,"connection")});
