...
(albumResult.tagObjects[a].customGenre||!1)+'"><span>'+b+'</span><span class="delete-tag-button ml-1" data-tag-id="'+d+"-"+c+'">&#10005;</span></a>'):$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag other-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].tag)+'" data-custom-genre="'+(albumResult.tagObjects[a].customGenre||!1)+'"><span>'+b+"</span></a>")}$(".delete-tag-button").click(function(a){a.preventDefault();deleteTag($(this).data("tag-id"))});
$(".album-tag").click(function(a){a.preventDefault();selectTag(document.getElementById($(this).attr("id")),a)})}}function selectTag(a,b){b&&b.preventDefault();a=document.getElementById(a.id);a.classList.toggle("badge-primary");a.classList.toggle("selected-tag");a.classList.toggle("badge-light");modifySelectedTags(escapeHtml(a.dataset.rawtag))}function modifySelectedTags(a){-1===$.inArray(a,selectedTags)?selectedTags.push(a):(a=selectedTags.indexOf(a),selectedTags.splice(a,1))}
...
$("#list-options").keyup(function(a){13===a.keyCode&&$("#add-to-list-button").click()});$("#search-for-second-album").click(function(){var a=window.open(window.location.protocol+"//"+window.location.host+"/search","_blank");a.location});$("#info-card .nav-link").click(function(a){a.preventDefault();toggleActiveInfoTab($(this))});
$("#tag-search-button").click(function(a){a.preventDefault();0<selectedTags.length?(a=new URL(document.location),a.pathname="/list",a.searchParams.set("type","tagsearch"),a.searchParams.set("search",selectedTags),window.location=a.href):alert("Select one or more tags to preform a tag search")});$("#clear-tag-button").click(function(a){a.preventDefault();clearTagArray(a)});$("#add-tag-button").click(function(a){a.preventDefault();addTag()});$("#add-tag-input").keyup(function(a){13===a.keyCode&&$("#add-tag-button").click()});
$("#add-connection-button").click(function(a){a.preventDefault();a=$("#add-connection-input").val().trim();$("#no-results-message").remove();$("#connection-search-results").html("");$("#connection-loader").show();executeSearch(a,"connection")});$("#add-connection-input").keyup(function(a){13===a.keyCode&&$("#add-connection-button").click()});$("#add-to-list-button").click(function(a){a.preventDefault();a=document.getElementById("list-options");a=a[a.selectedIndex].value;"Add to a list..."!==a&&addToList(a)});
...
$("#new-display-name").val("")):"MY FAVORITES"===a.toUpperCase()?(alert("'My Favorites' is a reserved list name. Give the existing 'My Favorites' functionality a shot or choose a different title."),$("#new-list-title").val("")):60<a.length?alert("List titles must be shorter than 60 characters in length."):30<b.length?alert("Display names must be shorter than 30 characters in length."):(addToNewList(escapeHtml(a),escapeHtml(b)),document.getElementById("new-display-name").value="",document.getElementById("new-list-title").value=
""):alert("All lists require a title!")});$("#custom-genre-checkbox-text").click(function(a){a.preventDefault();$("#custom-genre-checkbox").click()});$(document).ready(function(){var a=!1;"ontouchstart"in document.documentElement&&(a=!0);a&&(a=document.getElementById("connection-search-results"),a.style.paddingBottom="0px",a.style.overflowX="scroll")});
