...
a+"</li>")});$("#apple-album-id span").text(albumResult.appleAlbumID);$("#more-by-this-artist").click(function(a){a.preventDefault();$("#searchModal").modal("show");$("#search-modal-input").val(albumResult.artist);executeSearch(albumResult.artist)});$("#apple-music-link").click(function(a){a.preventDefault();return window.open(albumResult.appleURL,"_blank").location});populateTags();populateConnections();getListsWithAlbum(a);getUserLists();a?(updateTagDisplay(a),updateConnectionDisplay(a)):(0===$(".album-tag").length&&
$("#current-tags").html('<div class="text-primary text-center"><small>There are currently no tags for this album. Log in to start adding your own tags!</small></div>'),0===$(".connection").length&&$("#connected-albums").html('<div class="text-primary text-center"><small>There are currently no connections for this album. Log in to start adding your own connections!</small><br/><br/></div>'))}
...
function getUserLists(){$.ajax({method:"GET",url:"/api/v1/list/user/"+userID,success:function(a){userLists=a.message?[]:a;populateUserLists()}})}function getListsWithAlbum(a){$.ajax({method:"GET",url:"/api/v1/list/album/"+albumResult.appleAlbumID,success:function(b){listsWithAlbum=b.message||""===b?[]:b;populateListsWithAlbum(a)}})}
function populateListsWithAlbum(a){$("#all-lists").html("");$(".list-message").remove();userID&&albumResult.favorites&&albumResult.favorites.find(function(a){return a.userID===userID})&&$("#all-lists").append('<li class="list my-list" data-creator="'+userID+'"><a href="/list?type=myfavorites">My Favorites</a><span class="text-secondary"> by: You!</span><span class="remove-from-list-button" data-list-type="myfavorites">&#10005;</span></li>');listsWithAlbum.forEach(function(a){if(!a.isPrivate){var b=
...
alert(a.message):(userLists.push(a),listsWithAlbum.push(a),populateUserLists(),populateListsWithAlbum(),updateListDisplay(),$("#updateListModal").modal("hide"),$("#new-list-title").val(""),$("#new-display-name").val(""))}}))}
function removeFromList(a){var b=listsWithAlbum.find(function(b){return b.id===a});confirm('Are you sure you want to remove this album from the "'+b.title+'" list? You cannot undo this operation.')&&$.ajax({method:"PUT",url:"/api/v1/list/"+b.id,contentType:"application/json",data:JSON.stringify({method:"remove album",appleAlbumID:albumResult.appleAlbumID,title:albumResult.title,artist:albumResult.artist,releaseDate:albumResult.releaseDate,cover:albumResult.cover}),success:function(a){a=listsWithAlbum.indexOf(b);
listsWithAlbum.splice(a,1);populateListsWithAlbum();updateListDisplay()}})}function updateListDisplay(){return"My Lists"===sessionStorage.getItem("lists")?displayMyLists():displayAllLists()}
...
function populateTags(){clearTagArray();if(albumResult.tagObjects){$("#current-tags").html("");for(var a=0;a<albumResult.tagObjects.length;a++){var b=albumResult.tagObjects[a].text,c=albumResult.tagObjects[a].creator,d=void 0;d=parseInt(b)?"tag_".concat(b).replace(/[^A-Z0-9]+/ig,""):b.replace(/[^A-Z0-9]+/ig,"");c===userID?$("#current-tags").append('<a href="" id="'+d+"-"+c+'" class="badge badge-light album-tag my-tag" data-creator="'+c+'" data-rawtag="'+escapeHtml(albumResult.tagObjects[a].text)+
