...
function populateSearchModalResults(a,b){var c=0;!b||2>b?($("#search-modal-results").html(""),$("#searchModal .new-loader").hide()):(c=25*(b-1),$("#search-modal-placeholder").remove());if(a.albums){for(var d=0;d<a.albums.length;d++){var f=a.albums[d],e=d+c;createModalCard(e);populateModalCard(f,e)}$("#search-modal-results").append('<div id="search-modal-placeholder">&nbsp;</div>');25===a.albums.length&&(a=$("#search-modal-input").val().trim().replace(/[^\w\s]/gi,""),b++,addNextPageButton(a,b))}}
...
f+"</span>");$("#searchModalCard"+b+" .search-modal-card-image").attr("src",a.cover.replace("{w}",260).replace("{h}",260));$("#searchModalCard"+b+" .search-modal-card-album-link").attr("href","/album/"+a.appleAlbumID)}$("#search-modal-button").click(function(a){a.preventDefault();a=$("#search-modal-input").val().trim().replace(/[^\w\s]/gi,"");$("#search-modal-results").html("");$("#searchModal .new-loader").show();executeSearch(a)});$("#search-modal-input").keyup(function(a){13===a.keyCode&&$("#search-modal-button").click()});
$(document).ready(function(){var a=!1;"ontouchstart"in document.documentElement&&(a=!0);a&&(a=document.getElementById("search-modal-results"),a.style.paddingBottom="0px",a.style.overflowX="scroll")});
