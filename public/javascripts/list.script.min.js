...
function isGenre(a){return-1!=="Metalcore;Pop Punk;Emo;Rock;Post-Hardcore;Accoustic;Screamo;Metal;Nu Metal;Alt Metal;Djent;Accoustic;Jazz;Ska;Rap-Rock;Progressive;Punk;Rap".split(";").indexOf(a)}function removeFromArray(a,b){-1!==a.indexOf(b)&&a.splice(a.indexOf(b),1)}function addToArray(a,b){-1===a.indexOf(b)&&a.push(b)}var listData,listType=(new URL(document.location)).searchParams.get("type"),listID=(new URL(document.location)).searchParams.get("id");
function getList(){"favorites"===listType?$.ajax({method:"GET",url:"/api/v1/list/favorites/"+listID,success:function(a){listData=a;$("#main-page-loader").hide();populateList()}}):$.ajax({method:"GET",url:"/api/v1/list/"+listID,success:function(a){listData=a;$("#main-page-loader").hide();populateList()}})}
...
$("#no-albums-message").hide():$("#no-albums-message").show();var b=0;a.forEach(function(a){b+=1;createCard(b);populateCard(a,b)});$(".album-delete-button").on("click",function(){listData.user!==userID?alert("Sorry! Only the list creator can delete albums."):"userlist"===listType?removeAlbum($(this).attr("data-album-id")):alert("Sorry! This type of list won't let you delete albums.")})}else $("#no-albums-message").show();displayButtons()}
...
function populateAddToListModalResults(a){$("#add-album-search-results").html("");$("#add-album-card-body .new-loader").hide();if(a.albums&&0<a.albums.length){for(var b=0;b<a.albums.length;b++){var d=a.albums[b],c=b;createAddAlbumModalCard(c);populateAddAlbumModalCard(d,c)}$("#add-album-search-results").append('<div id="add-search-modal-placeholder">&nbsp;</div>');addAlbumResults=a.albums}else $("#add-album-search-results").after('<div id="no-results-message" class="text-primary" style="text-align:center;">It looks like no albums matched your search terms. Try a different search!</div>')}
...
function populateAddAlbumModalCard(a,b){var d=32<a.artist.length?truncate(a.artist,32):a.artist,c=49<a.artist.length?truncate(a.artist,49):a.artist,e=44<a.title.length?truncate(a.title,44):a.title,f=66<a.title.length?truncate(a.title,66):a.title;$("#addAlbumModalCard"+b+" .search-modal-card-title").html('<span class="search-modal-card-large-artist">'+c+'</span><span class="search-modal-card-small-artist">'+d+"</span>");$("#addAlbumModalCard"+b+" .search-modal-card-album").html('<span class="search-modal-card-large-album">'+
...
function getCleanAlbumArray(a){var b=[];a.forEach(function(a){a.album?b.push(a.album):b.push(a)});b.forEach(function(a){a.tagObjects&&a.tagObjects.forEach(function(b){b.creator===listData.user&&isGenre(b.tag)&&-1===a.genres.indexOf(b.tag)&&a.genres.push(b.tag)});a.year=a.releaseDate.slice(0,4);a.genres&&(a.genres=a.genres.join("||").toLowerCase().split("||"))});return b}
...
function filterAlbums(){var a=getFilterObject(),b=getCleanAlbumArray(listData.albums);a.genre&&(b=b.filter(function(b){return b.genres?-1!==b.genres.indexOf(a.genre):!1}));a.artist&&(b=b.filter(function(b){return!!b.artist&&b.artist.toLowerCase()===a.artist}));a.year&&(b=b.filter(function(b){return!!b.year&&b.year===a.year}));bubbleSort(b,"releaseDate");return b=b.reverse()}
...
$("#editListModal .nav-link").click(function(a){a.preventDefault();toggleActiveInfoTab($(this))});$("#to-top-button").click(function(a){a.preventDefault();scrollToTop()});var config={apiKey:"AIzaSyAoadL6l7wVMmMcjqqa09_ayEC8zwnTyrc",authDomain:"album-tags-v1d1.firebaseapp.com",projectId:"album-tags-v1d1"},defaultApp=firebase.initializeApp(config),userID=!1;
