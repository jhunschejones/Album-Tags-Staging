...
function isGenre(a){return-1!=="Metalcore;Pop Punk;Emo;Rock;Post-Hardcore;Accoustic;Screamo;Metal;Nu Metal;Alt Metal;Djent;Accoustic;Jazz;Ska;Rap-Rock;Progressive;Punk;Rap".split(";").indexOf(a)}function removeFromArray(a,b){-1!==a.indexOf(b)&&a.splice(a.indexOf(b),1)}function addToArray(a,b){-1===a.indexOf(b)&&a.push(b)}function removeDuplicates(a){for(var b=[],c=0;c<a.length;c++)-1==b.indexOf(a[c])&&b.push(a[c]);return b}var listData,listType,listID,startingURL=new URL(document.location);
...
$("#no-albums-message").hide():(clearFilters(),a=filterAlbums(),0<a.length?($("#no-albums-message").hide(),$("#albums").html("")):$("#no-albums-message").show());var b=0;a.forEach(function(a){b+=1;createCard(b);populateCard(a,b)});$(".album-delete-button").on("click",function(){listData.user!==userID?alert("Sorry! Only the list creator can delete albums."):"userlist"===listType?removeListAlbum($(this).attr("data-album-id")):"myfavorites"===listType?removeFavoritesAlbum($(this).attr("data-album-id")):
alert("Sorry! This type of list won't let you delete albums.")});populateFilters(a)}else $("#no-albums-message").show(),populateFilters([]);displayButtons()}
...
32):a)+"</span>")});$(".artist-filter").click(function(a){a.preventDefault();toggleFilter("artist",$(this).data("artist"))});var e=[];a.forEach(function(a){a.tagGenres&&a.tagGenres.forEach(function(a){addToArray(e,a)})});1>e.length?$("#filter-by-genre-button").hide():$("#filter-by-genre-button").show();e.sort();e.forEach(function(a){b.genre&&a===b.genre?$("#genre-filter-dropdown").append('<span class="badge badge-primary genre-filter" data-genre="'+a+'">'+a+"</span>"):b.genre||$("#genre-filter-dropdown").append('<span class="badge badge-light genre-filter" data-genre="'+
a+'">'+a+"</span>")});$(".genre-filter").click(function(a){a.preventDefault();toggleFilter("genre",$(this).data("genre"))})}}function toggleFilter(a,b){var c=new URL(document.location);c.searchParams.get(a)==b?c.searchParams.delete(a):c.searchParams.set(a,b);history.replaceState({},"",c);populateList()}function clearFilters(){var a=new URL(document.location);a.searchParams.delete("year");a.searchParams.delete("artist");a.searchParams.delete("genre");history.replaceState({},"",a);populateList()}$("#clear-filters-button").click(clearFilters);
var addFavoritesAlbumResults=[];
function populateAddToFavoritesModalResults(a){$("#favorites-search-results").html("");$("#addFavoritesAlbumModal .new-loader").hide();if(a.albums&&0<a.albums.length){for(var b=0;b<a.albums.length;b++){var c=a.albums[b],d=b;createFavoritesModalCard(d);populateFavoritesModalCard(c,d)}$("#favorites-search-results").append('<div id="add-search-modal-placeholder">&nbsp;</div>');addFavoritesAlbumResults=a.albums}else $("#favorites-search-results").after('<div id="no-results-message" class="text-primary mb-3" style="text-align:center;">It looks like no albums matched your search terms. Try a different search!</div>')}
...
function populateFavoritesModalCard(a,b){var c=32<a.artist.length?truncate(a.artist,32):a.artist,d=49<a.artist.length?truncate(a.artist,49):a.artist,e=44<a.title.length?truncate(a.title,44):a.title,f=66<a.title.length?truncate(a.title,66):a.title;$("#addFavoritesModalCard"+b+" .search-modal-card-title").html('<span class="search-modal-card-large-artist">'+d+'</span><span class="search-modal-card-small-artist">'+c+"</span>");$("#addFavoritesModalCard"+b+" .search-modal-card-album").html('<span class="search-modal-card-large-album">'+
f+'</span><span class="search-modal-card-small-album">'+e+"</span>");$("#addFavoritesModalCard"+b+" .search-modal-card-image").attr("src",a.cover.replace("{w}",260).replace("{h}",260));$("#addFavoritesModalCard"+b).click(function(a){a.preventDefault();a=$(this).data("result-index");var b=addFavoritesAlbumResults[a];listData.albums.find(function(a){return a.appleAlbumID===b.appleAlbumID})?alert('"'+b.title+'" is already in your favorites.'):addToFavorites(b)})}
$("#add-favorites-album-button").click(function(a){a.preventDefault();a=$("#add-favorites-album-input").val().trim().replace(/[^\w\s]/gi,"");$("#no-results-message").remove();$("#favorites-search-results").html("");$("#addFavoritesAlbumModal .new-loader").show();executeSearch(a,"add to favorites")});$("#add-favorites-album-input").keyup(function(a){13===a.keyCode&&$("#add-favorites-album-button").click()});
function addToFavorites(a){a&&userID&&$.ajax("/api/v1/album/favorites/new",{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,albumData:a}),success:function(a){a.message?alert(a.message):(listData.albums.push(a),$("#addFavoritesAlbumModal").modal("hide"),$("#add-favorites-album-input").val(""),$("#favorites-search-results").html(""),populateList())}})}
function removeFavoritesAlbum(a){if(listData.user!==userID)alert("Sorry, only the list creator can delete albums from a list.");else{var b=listData.albums.find(function(b){return b.appleAlbumID===a});confirm('Are you sure you want to remove "'+b.title+'" from your favorites? You cannot undo this operation.')&&$.ajax("/api/v1/album/favorites/"+a,{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:userID}),success:function(a){a.message?alert(a.message):(removeFromArray(listData.albums,
b),populateList())}})}}
...
3E3)},function(){})}})});$("#add-album-modal-button").click(function(a){a.preventDefault();a=$("#add-album-modal-input").val().trim().replace(/[^\w\s]/gi,"");$("#add-album-search-results").html("");$("#no-results-message").remove();$("#add-album-card-body .new-loader").show();executeSearch(a,"add to list")});document.getElementById("share-favorites-button").addEventListener("click",function(){$("#shareFavoritesModal").modal("show")});$("#add-album-modal-input").keyup(function(a){13===a.keyCode&&$("#add-album-modal-button").click()});
...
$("#add-album-button").click(function(a){a.preventDefault();listData.user!==userID?alert("Sorry! Only the list creator can add albums to this list."):"userlist"===listType?(toggleActiveInfoTab($("#add-album-modal-nav-tab")),$("#editListModal").modal("show"),$("#list-title-input").val(listData.title),$("#list-display-name-input").val(listData.displayName||"Unknown")):"myfavorites"===listType&&$("#addFavoritesAlbumModal").modal("show")});
$(".add-album-refrence").click(function(a){a.preventDefault();$("#pageInfoModal").modal("hide");$("#add-album-button").click()});$(".edit-list-refrence").click(function(a){a.preventDefault();$("#pageInfoModal").modal("hide");$("#edit-button").click()});$("#list-title-input").keyup(function(a){13===a.keyCode&&$("#update-list-title").click()});$("#list-display-name-input").keyup(function(a){13===a.keyCode&&$("#update-list-display-name").click()});
document.getElementById("update-list-display-name").addEventListener("click",editDisplayName);document.getElementById("update-list-title").addEventListener("click",editListTitle);$("#editListModal .nav-link").click(function(a){a.preventDefault();toggleActiveInfoTab($(this))});$("#to-top-button").click(scrollToTop);$(document).on("scroll",function(){$("#year-filter-dropdown").removeClass("show");$("#genre-filter-dropdown").removeClass("show");$("#artist-filter-dropdown").removeClass("show")});
