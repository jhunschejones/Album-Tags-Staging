...
function getCleanAlbumArray(a){var b=[];a.forEach(function(a){a.album?b.push(a.album):b.push(a)});b.forEach(function(a){a.tagObjects&&a.tagObjects.forEach(function(b){b.creator===listData.user&&isGenre(b.tag)&&-1===a.genres.indexOf(b.tag)&&a.genres.push(b.tag)});a.year=a.releaseDate.slice(0,4)});return b}
...
function filterAlbums(){var a=getFilterObject(),b=getCleanAlbumArray(listData.albums);a.genre&&(b=b.filter(function(b){return b.genres?-1!==b.genres.indexOf(a.genre):!1}));a.artist&&(b=b.filter(function(b){return!!b.artist&&b.artist===a.artist}));a.year&&(b=b.filter(function(b){return!!b.year&&b.year===a.year}));bubbleSort(b,"releaseDate");return b=b.reverse()}
function populateFilters(a){var b=getFilterObject();$("#year-filter-dropdown").html("");$("#artist-filter-dropdown").html("");$("#genre-filter-dropdown").html("");if(1>a.length)$(".remove-if-no-albums").hide();else{$(".remove-if-no-albums").show();var c=[];a.forEach(function(a){addToArray(c,a.releaseDate.slice(0,4))});c.sort();c.forEach(function(a){b.year&&a===b.year?$("#year-filter-dropdown").append('<span class="badge badge-primary year-filter" data-year="'+a+'">'+a+"</span>"):$("#year-filter-dropdown").append('<span class="badge badge-light year-filter" data-year="'+
...
32):a)+"</span>")});$(".artist-filter").click(function(a){a.preventDefault();toggleFilter("artist",$(this).data("artist"))});var e=[];a.forEach(function(a){a.genres&&a.genres.forEach(function(a){isGenre(a)?addToArray(e,a):console.log("Ignored genre: '"+a+"'")})});1>e.length?$("#filter-by-genre-button").hide():$("#filter-by-genre-button").show();e.sort();e.forEach(function(a){b.genre&&a===b.genre?$("#genre-filter-dropdown").append('<span class="badge badge-primary genre-filter" data-genre="'+a+'">'+
a+"</span>"):$("#genre-filter-dropdown").append('<span class="badge badge-light genre-filter" data-genre="'+a+'">'+a+"</span>")});$(".genre-filter").click(function(a){a.preventDefault();toggleFilter("genre",$(this).data("genre"))})}}function toggleFilter(a,b){var c=new URL(document.location);c.searchParams.get(a)==b?c.searchParams.delete(a):c.searchParams.set(a,b);history.pushState({},"",c);populateList()}
function clearFilters(){var a=new URL(document.location);a.searchParams.delete("year");a.searchParams.delete("artist");a.searchParams.delete("genre");history.pushState({},"",a);populateList()}$("#clear-filters-button").click(clearFilters);
