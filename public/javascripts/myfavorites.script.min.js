...
function masterFilter(a){if("none"!=a)for(var b=0;b<albumCardsList.length;b++){var c=albumCardsList[b];$(c).hasClass(a)||(c.style.display="none")}}function restoreCards(){for(var a=0;a<albumCardsList.length;a++)albumCardsList[a].style.display="inline"}function clearFilters(){filterArtist=filterGenre=filterYear="none";filterTopTen&&toggleStar();restoreCards();whatsOnThePage_genres();whatsOnThePage_years();whatsOnThePage_artists();whatsOnThePage_Top10()}
$(document).on("scroll",function(){$("#year_filter_menu").removeClass("show");$("#genre_filter_menu").removeClass("show")});function whatsOnThePage_artists(){for(var a=$(".albumCard:visible"),b=[],c=0;c<a.length;c++){var d=$("#"+a[c].id).prop("classList")[3];b.push(d)}a=$(".artist_to_filter_by");for(c=0;c<a.length;c++)d=a[c].id,-1==b.indexOf(d)?$("#"+d).hide():$("#"+d).show()}
...
function startFavoritesPage(){$("#all_cards").html("");if(myFavoriteAlbums.message)showDOMelement("log_in_message"),$("#log_in_message").html("<div style='text-align:center;'><p style='margin: 20px 5px 50px 5px;'>Looks like you don't have any favorites yet! <a href='/search'>Search</a> for an album then click the <img src='../images/heart-unliked.png' height='30' width='auto'> icon on the Lists tab to add it to your favorites.</p></div>"),hideDOMelement("filter_by_genre_dropdown_button"),hideDOMelement("filter_by_year_dropdown_button"),
hideDOMelement("filter_by_artist_dropdown_button"),hideDOMelement("clear_filters_button"),hideDOMelement("share-favorites-button"),hideDOMelement("to_top_button");else{$("#log_in_message").html("");$("#artist_filter_menu").html("");$("#artist_filter_menu").append('<small id="loading_artist_filters" class="text-primary" style="margin-left:8px;">Loading Artist Filters...</small>');bubbleSort(myFavoriteAlbums,"releaseDate");myFavoriteAlbums=myFavoriteAlbums.reverse();for(var a=0;a<myFavoriteAlbums.length;a++){var b=
myFavoriteAlbums[a],c=a+1;createCard(b,c);populateCard(b,c)}$(".remove-favorite-button").click(function(a){a.preventDefault();a=$(this).data("album-id");removeFromFavorites(a)});albumCardsList=$(".albumCard");startTags()}}
...
function addInfoButtons(){$("#compact_menu p").width("80%");$("#compact_menu p").css("margin-top","-5px");$(".my_dropdown").css("margin-top","-2px");$("#compact_menu p").append('<span class="text-secondary ml-2 mr-1" style="cursor:pointer;font-size:1.2em;" data-toggle="modal" data-target="#addFavoritesAlbumModal">&plus;</span>');$("#compact_menu p").append('<span class="text-secondary ml-2 float-right page-info-button" style="cursor:pointer;margin-top:3px;" data-toggle="modal" data-target="#pageInfoModal">&#9432;</span>');
$("#all-favorites-filters").append('<button class="btn btn-sm btn-outline-secondary sticky-top float-left button_text" style="cursor:pointer;margin-top:2px;" data-toggle="modal" data-target="#addFavoritesAlbumModal">Add Album</button>');$("#all-favorites-filters").append('<button class="btn btn-sm btn-outline-secondary sticky-top float-left button_text" style="cursor:pointer;margin-top:2px;" data-toggle="modal" data-target="#pageInfoModal">&#9432;</button>')}addInfoButtons();
$(".add-album-instruction-link").click(function(a){a.preventDefault();$("#pageInfoModal").modal("hide");$("#addFavoritesAlbumModal").modal("show")});
