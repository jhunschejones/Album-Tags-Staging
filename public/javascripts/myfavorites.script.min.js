...
logoutButton2.addEventListener("click",logOut);$(".login_button").on("click",logIn);var myFavoriteAlbums;function getFavoriteAlbums(){$.ajax({method:"GET",url:"/api/v1/album/favorites/"+userID,success:function(a){a.message&&"This user does not have any favorited albums."===a.message?(myFavoriteAlbums=[],startFavoritesPage()):a.message?alert(a.message):(myFavoriteAlbums=a,startFavoritesPage())}})}
var filterYear="none",filterGenre="none",filterArtist="none",filterTopTen=!1,albumCardsList,yearsList=[],appleGenreList=[],yearsOnPage=[],artistsList=[];function masterFilter(a){if("none"!=a)for(var b=0;b<albumCardsList.length;b++){var c=albumCardsList[b];$(c).hasClass(a)||(c.style.display="none")}}function restoreCards(){for(var a=0;a<albumCardsList.length;a++)albumCardsList[a].style.display="inline-flex"}
function clearFilters(){filterArtist=filterGenre=filterYear="none";filterTopTen&&toggleStar();restoreCards();whatsOnThePage_genres();whatsOnThePage_years();whatsOnThePage_artists();whatsOnThePage_Top10()}$(document).on("scroll",function(){$("#year_filter_menu").removeClass("show");$("#genre_filter_menu").removeClass("show");$("#artist_filter_menu").removeClass("show")});
function whatsOnThePage_artists(){for(var a=$(".albumCard:visible"),b=[],c=0;c<a.length;c++){var d=$("#"+a[c].id).prop("classList")[3];b.push(d)}a=$(".artist_to_filter_by");for(c=0;c<a.length;c++)d=a[c].id,-1==b.indexOf(d)?$("#"+d).hide():$("#"+d).show()}
...
function buildYearFilters(){$("#year_filter_menu").html("");$("#year_filter_menu").append('<small id="loading_year_filters" class="text-primary" style="margin-left:8px;">Loading Year Filters...</small>');yearsList=removeDuplicates(yearsList);yearsList.sort().reverse();for(var a=0;a<yearsList.length;a++){var b=yearsList[a];filterYear=="year-"+b?$("#year_filter_menu").append('<a id="year-'+b+'" class="badge badge-primary year_to_filter_by" href="" onclick="filterByYear('+b+', event)">'+b+"</a>"):$("#year_filter_menu").append('<a id="year-'+
b+'" class="badge badge-light year_to_filter_by" href="" onclick="filterByYear('+b+', event)">'+b+"</a>")}$("#loading_year_filters").hide();whatsOnThePage_years()}function filterByYear(a,b){b&&b.preventDefault();filterYear=document.getElementById("year-"+a).classList.contains("badge-primary")?"none":"year-"+a;restoreCards();masterFilter(filterGenre);masterFilter(filterYear);masterFilter(filterArtist);filterByTopTen(filterTopTen);whatsOnThePage_Top10()}var genresList=[];
...
genresList[a];filterGenre=="genre-"+replaceSpaceWithUnderscore(replaceSepecialCharacters(b))?$("#genre_filter_menu").append('<a id="genre-'+replaceSpaceWithUnderscore(replaceSepecialCharacters(b))+'" class="badge badge-primary genre_to_filter_by" href="" onclick="filterByGenre(\''+b+"', event)\">"+b+"</a>"):$("#genre_filter_menu").append('<a id="genre-'+replaceSpaceWithUnderscore(replaceSepecialCharacters(b))+'" class="badge badge-light genre_to_filter_by" href="" onclick="filterByGenre(\''+b+"', event)\">"+
b+"</a>")}$("#loading_genre_filters").hide();whatsOnThePage_genres()}function filterByGenre(a,b){b&&b.preventDefault();filterGenre=document.getElementById("genre-"+replaceSpaceWithUnderscore(replaceSepecialCharacters(a))).classList.contains("badge-primary")?"none":"genre-"+replaceSpaceWithUnderscore(replaceSepecialCharacters(a));restoreCards();masterFilter(filterGenre);masterFilter(filterYear);masterFilter(filterArtist);filterByTopTen(filterTopTen);whatsOnThePage_Top10()}
...
c+'" class="badge badge-primary artist_to_filter_by" href="" onclick="filterByArtist(\''+c+"', event)\">"+b+"</a>"):$("#artist_filter_menu").append('<a id="artist-'+c+'" class="badge badge-light artist_to_filter_by" href="" onclick="filterByArtist(\''+c+"', event)\">"+b+"</a>")}$("#loading_artist_filters").hide();whatsOnThePage_artists()}
function filterByArtist(a,b){b&&b.preventDefault();a=replaceSpaceWithUnderscore(removeDoubleSpace(replaceSepecialCharacters(a)));filterArtist=document.getElementById("artist-"+a).classList.contains("badge-primary")?"none":"artist-"+a;restoreCards();masterFilter(filterGenre);masterFilter(filterYear);masterFilter(filterArtist);filterByTopTen(filterTopTen);whatsOnThePage_Top10()}
...
function populateAddToFavoritesModalResults(a){$("#favorites-search-results").html("");$("#addFavoritesAlbumModal .new-loader").hide();if(a.albums&&0<a.albums.length){for(var b=0;b<a.albums.length;b++){var c=a.albums[b],d=b;createFavoritesModalCard(d);populateFavoritesModalCard(c,d)}createFavoritesModalCard(a.albums.length+1);addAlbumResults=a.albums}else $("#favorites-search-results").after('<div id="no-results-message" class="text-primary mb-3" style="text-align:center;">It looks like no albums matched your search terms. Try a different search!</div>')}
...
function addToFavorites(a){a&&userID&&$.ajax("/api/v1/album/favorites/new",{method:"POST",contentType:"application/json",data:JSON.stringify({user:userID,albumData:a}),success:function(a){a.message?alert(a.message):(myFavoriteAlbums.push(a),$("#addFavoritesAlbumModal").modal("hide"),startFavoritesPage(),$("#add-favorites-album-input").val(""),$("#favorites-search-results").html(""),restoreCards(),masterFilter(filterGenre),masterFilter(filterYear),masterFilter(filterArtist),filterByTopTen(filterTopTen),
whatsOnThePage_Top10(),1>$(".albumCard:visible").length&&clearFilters())}})}
function startFavoritesPage(){$("#all_cards").html("");if(1>myFavoriteAlbums.length)showDOMelement("log_in_message"),$("#log_in_message").html("<div class='text-secondary' style='text-align:center;'><p style='margin: 20px 5px 50px 5px;'>Looks like you don't have any favorites yet! Click <span class='text-primary' style='cursor:pointer;' data-toggle='modal' data-target='#addFavoritesAlbumModal'>Add Album</span> to find an album and add it to your favorites.</p></div>"),hideDOMelement("filter_by_genre_dropdown_button"),
hideDOMelement("filter_by_year_dropdown_button"),hideDOMelement("filter_by_artist_dropdown_button"),hideDOMelement("clear_filters_button"),hideDOMelement("share-favorites-button"),hideDOMelement("to_top_button"),whatsOnThePage_Top10();else{$("#log_in_message").hide();showDOMelement("filter_by_genre_dropdown_button");showDOMelement("filter_by_year_dropdown_button");showDOMelement("filter_by_artist_dropdown_button");showDOMelement("clear_filters_button");showDOMelement("share-favorites-button");showDOMelement("to_top_button");
$("#artist_filter_menu").html("");$("#artist_filter_menu").append('<small id="loading_artist_filters" class="text-primary" style="margin-left:8px;">Loading Artist Filters...</small>');bubbleSort(myFavoriteAlbums,"releaseDate");myFavoriteAlbums=myFavoriteAlbums.reverse();for(var a=0;a<myFavoriteAlbums.length;a++){var b=myFavoriteAlbums[a],c=a+1;createCard(b,c);populateCard(b,c)}$(".remove-favorite-button").click(function(a){a.preventDefault();a=$(this).data("album-id");removeFromFavorites(a)});albumCardsList=
$(".albumCard");startTags()}}
function removeFromFavorites(a){var b=myFavoriteAlbums.find(function(b){return b._id===a});confirm('Are you sure you want to remove "'+b.title+'" from your favorites? You cannot undo this operation.')&&$.ajax("/api/v1/album/favorites/"+a,{method:"DELETE",contentType:"application/json",data:JSON.stringify({user:userID}),success:function(a){a.message?alert(a.message):(removeElementFromArray(myFavoriteAlbums,b),startFavoritesPage(),restoreCards(),masterFilter(filterGenre),masterFilter(filterYear),masterFilter(filterArtist),
filterByTopTen(filterTopTen),whatsOnThePage_Top10(),1>$(".albumCard:visible").length&&clearFilters())}})}$(document).ready(function(){var a=!1;"ontouchstart"in document.documentElement&&(a=!0);a&&(a=document.getElementById("favorites-search-results"),a.style.paddingBottom="0px",a.style.overflowX="scroll")});
